import { Zap, Code, Video, DollarSign, Cpu, Layers } from "lucide-react";

const FEATURES = [
  {
    icon: Zap,
    title: "Atendimento Instantâneo On-Demand",
    description: "Abra um pedido de SOS especificando sua stack e receba atendimento de mentores qualificados em menos de 2 minutos.",
  ...
  },
  {
    ...
    description: "A mesma engine do VS Code com sincronização atômica em tempo real via Yjs. Digitem no mesmo arquivo simultaneamente sem conflitos.",
    badge: "Yjs + Monaco",
    color: "text-indigo-400 bg-indigo-500/10 border-indigo-500/30",
  },
  {
    icon: Video,
    title: "Videoconferência HD e Compartilhamento",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {FEATURES.map((feat) => {
            ...
            return (
              <div
                key={feat.title}
                className="glass-card p-6 rounded-3xl border border-white/10 hover:border-indigo-500/40 transition duration-300 space-y-4 flex flex-col justify-between group"
              >
